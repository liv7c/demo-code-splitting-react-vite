...
function Movies() {
  const {data: movies, isLoading, isError, isSuccess} = useMovies()

  return (
    <>
      <h1 className="mb-4 text-xl">Our most popular movies</h1>
      {isLoading ? <p className="py-3 text-sm">Loading movies...</p> : null}
      {isError ? <p className="text-red-700">An error occurred!</p> : null}
      {isSuccess && (
